...
# dash_app = Dash(__name__, server=app, url_base_pathname=f'/dash/')
...
def view(city):
    try:
        ...
        fig.update_layout(title=f"Weather Forecast for {city}",
                            xaxis_title="Date",
                            yaxis_title="Value",
...
def check_weather():
    ...
    return render_template('result.html', start=start, end=end,city1_weather=city1_weather, city2_weather=city2_weather,stops=stops, stops_weather=stops_weather, result=result, count=len(stops))
